//(function(b){b.fn.audioPlayer=function(e){var a={player:{self:".jplayer_wr",title:"#song_title",duration:"#play_time",currentTime:"#total_time",play:"#jp_play",pause:"#jp_pause",stop:"#jp_stop",prev:"#jp_prev",next:"#jp_next",playlistToggle:"#jp_playlist",volumeMin:"#jp_volume_min",volumeMax:"#jp_volume_max",volumeBar:"#jp_volume_bar",progressBar:"#jp_progress",close:"#jp_close"},playlist:{self:"#jplayer_playlist",audioTrack:".sortable_playlist"},soundsContainer:{self:".audio",audioTrack:".audio a"}};return this.each(function(){e&&b.extend(a,e);audioPlayer=new audioPlayer_obj(a)})};audioPlayer_obj=function(e){var a=this;a.player={self:"",title:"",duration:"",currentTime:"",play:"",pause:"",volumeBar:"",progressBar:""};a.playlist={self:"",audioTrack:"",addAudioTrack:function(c){var d="";b(a.soundsContainer.audioTrack).hasClass("ui-selected")?b(a.soundsContainer.audioTrack+":link.ui-selected").each(function(){d+='<a id="playlist_'+b(this).attr("id")+'"\nname="sync"\nclass="sortable_playlist ui-state-default ui-widget-content"\nhref="'+b(this).attr("href")+'"\nrel="'+b(this).attr("rel")+'">\n'+b(this).html()+"\n</a>"}):d='<a id="playlist_'+b(c).attr("id")+'"\nname="sync"\nclass="sortable_playlist ui-state-default ui-widget-content"\nhref="'+b(c).attr("href")+'"\nrel="'+b(c).attr("rel")+'">\n'+b(c).html()+"\n</a>";b(".jspPane").sortable({handle:".title",stop:function(b,c){variables.sortableOverDroppable||a.playlist.removeAudioTrack(c.item)}});b(".jspPane").disableSelection();b(".jspPane").selectable();b(this.self).find(".jspPane").append(d)},removeAudioTrack:function(c){b(a.playlist.audioTrack).hasClass("ui-selected")?b(a.playlist.audioTrack+":link.ui-selected").detach():b(a.playlist.self).find(c).detach()}};a.soundsContainer={self:"",audioTrack:""};a.duration="";a.isSongLoaded=!1;a.detach=!1;a.init=function(c){c&&(b.extend(a.player,c.player),b.extend(a.playlist,c.playlist),b.extend(a.soundsContainer,c.soundsContainer));a._create();a._volumeLevel(0.75)};a._create=function(){a.audio=new Audio;a.audio.canPlayOgg=!!a.audio.canPlayType&&""!=a.audio.canPlayType('audio/ogg; codecs="vorbis"');a.audio.canPlayMp3=!!a.audio.canPlayType&&""!=a.audio.canPlayType('audio/mpeg; codecs="mp3"');a.player.progressBarWidth=b(a.player.progressBar).width();a.player.progressBarLoad=b(a.player.progressBar).children();a.player.progressBarPlay=b(a.player.progressBar).children().children();a.player.volumeBarWidth=b(a.player.volumeBar).width();a.player.volumeLoad=b(a.player.volumeBar).children();a.audio.addEventListener("loadedmetadata",function(){var c=a._timeToMMSS(a.audio.duration);a.duration=a.audio.duration;b(a.player.duration).html(c);b(a.player.currentTime).html("");a._play();a.isSongLoaded=!0});a.audio.addEventListener("timeupdate",function(){var c=a._timeToMMSS(a.audio.currentTime);b(a.player.currentTime).html(c);c=parseInt(a.player.progressBarWidth)*(a.audio.currentTime/a.duration);a.player.progressBarPlay.width(c);a.isSongLoaded&&(c=a.audio.seekable.end(0)/a.duration,c*=a.player.progressBarWidth,a.player.progressBarLoad.width(c))});a.audio.addEventListener("ended",function(){a._stop();a._next()});b(a.playlist.audioTrack+", "+a.soundsContainer.audioTrack).live("click",function(c){c.preventDefault();c={title:b(this).text(),mp3:b(this).attr("href"),ogg:b(this).attr("rel")};b(this).parents(a.playlist.self)[0]&&(b(a.playlist.audioTrack).removeClass("active"),b(this).addClass("active"));a._change(c);b(a.player.self).is(":hidden")&&(b(a.player.self).fadeIn(),a.playlist.addAudioTrack(this),b(a.playlist.audioTrack).filter("#playlist_"+b(this).attr("id")).addClass("active"))});b(a.player.play).bind("click",function(){a._play()});b(a.player.pause).bind("click",function(){a._pause()});b(a.player.stop).bind("click",function(){a._stop()});b(a.player.prev).bind("click",function(){a._prev()});b(a.player.next).bind("click",function(){a._next()});b(a.player.playlistToggle).bind("click",function(){a._togglePlaylist()});b(a.player.close).bind("click",function(){a._closePlayer()});b(a.player.volumeMin).bind("click",function(){a._volumeLevel(0.001)});b(a.player.volumeMax).bind("click",function(){a._volumeLevel(1)});b(a.player.volumeBar).bind("click",function(c){var d=b(a.player.volumeBar).offset();c=(c.pageX-d.left)/parseInt(a.player.volumeBarWidth);a._volumeLevel(c)});b(a.player.progressBar).bind("click",function(b){a._progressBar(b)});return a.audio};a._timeToMMSS=function(a){var b=parseInt(a/60%60);a=parseInt(a%60);1==b.toString().length&&(b="0"+b);1==a.toString().length&&(a="0"+a);return b+":"+a};a._change=function(c){a.isSongLoaded=!1;a.audio.paused||a.audio.pause();a.audio.canPlayMp3?b(a.audio).prop("src",c.mp3):b(a.audio).prop("src",c.ogg);b(a.player.title).html(c.title)};a._play=function(c){c&&(a.audio.currentTime=c);a.audio.play();b(a.player.play).hide();b(a.player.pause).show()};a._pause=function(){a.audio.pause();b(a.player.pause).hide();b(a.player.play).show()};a._stop=function(){a._pause();a.audio.currentTime=0};a._prev=function(){if(0==b(a.playlist.audioTrack).filter(".active").index())var c=b(a.playlist.audioTrack).removeClass("active").last();else c=b(a.playlist.audioTrack).filter(".active").prev(),b(a.playlist.audioTrack).removeClass("active");void 0!=c[0]&&(c.addClass("active"),c={title:c.text(),mp3:c.attr("href"),ogg:c.attr("rel")},a._change(c))};a._next=function(){if(b(a.playlist.audioTrack).filter(".active").index()==b(a.playlist.audioTrack).length-1)var c=b(a.playlist.audioTrack).removeClass("active").first();else c=b(a.playlist.audioTrack).filter(".active").next(),b(a.playlist.audioTrack).removeClass("active");void 0!=c[0]&&(c.addClass("active"),c={title:c.text(),mp3:c.attr("href"),ogg:c.attr("rel")},a._change(c))};a._volumeLevel=function(b){a.player.volumeLoad.width(a.player.volumeBarWidth*b);a.audio.volume=b};a._progressBar=function(c){var d=b(a.player.progressBar).offset();c=c.pageX-d.left;d=c/a.player.progressBarWidth*a.duration;a.player.progressBarPlay.width(c);a._play(d)};a._togglePlaylist=function(){b(a.playlist.self).slideToggle()};a._closePlayer=function(){a._stop();b(a.player.self).fadeOut();b(a.playlist.audioTrack).removeClass("active")};a.init(e)}})(jQuery);
...
/*accordion.js*/
(function(b){b.fn.accordion=function(c){var e={mouseenter:!0},g=function(c){var a=b(this),e=b(".accordion"),d=b(".dropdown_teg > li > a"),a=a.find(".active");a.addClass("on").parents(".dropdown_teg").addClass("list-show");a.parents("li").addClass("on");a.children(".dropdown_teg").addClass("list-show");d.on("click",function(){var f=b(this).parent("li"),a=f.parent(".dropdown_teg"),c=f.children(".dropdown_teg"),a=a.children("li"),d=a.children(".dropdown_teg");e.find("li").removeClass("active");f.addClass("active");d.find("li").removeClass("active");c.addClass("list-show");a.removeClass("on");d.find("li").removeClass("on");f.addClass("on");a.not(".active").find(".dropdown_teg").removeClass("list-show")});c.mouseenter&&d.hover(function(a){b(this).hoverFlow(a.type,{marginLeft:10},250)},function(a){b(this).hoverFlow(a.type,{marginLeft:0},250)})};return this.each(function(){c&&b.extend(e,c);g.apply(this,[e])})}})(jQuery);

/*scrollToTop.js*/ 
$(function(a){a.fn.scrollToTop=function(){var b=a(this),c=a(window);c.on("scroll",function(){b[0===c.scrollTop()?"removeClass":"addClass"]("to-up-show")});b.on("click",function(){a("html, body").animate({scrollTop:0},"slow")})}}(jQuery));
